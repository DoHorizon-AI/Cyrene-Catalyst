...
class DirectPluginDataPreparationPort:
    ...
    def inspect(self, source: Path) -> SourceInspection:
        ...
        with _result_file() as result_path:
            receipt = self._invoke(
                "inspect",
                {"source_path": str(source.resolve()), "result_path": str(result_path)},
            )
            document = _read_verified_result(result_path, receipt)
        rows = _mapping_list(document.get("rows"), "rows")
        try:
            source_format = ImportFormat(_text(receipt.get("format"), "format"))
        ...
        return SourceInspection(source_format, rows, detected_fields, row_count)
    ...
    def transform(self, source: Path, destination: Path) -> EngineResult:
        """Execute JSONL-to-Parquet and verify the produced bytes."""
